...
def readFromStr(filename):
    ...
    fsvm = open('strPlusSeqSVMfileIP.txt' , 'w')
    count = 401
...
def plotROC(rocList):
    ...
    plt.title('Receiver operating characteristic seq + str per protein chain')
    plt.legend(loc="lower right")
    plt.savefig('ROC for Structure + Sequence per protein chain')
    plt.show()
